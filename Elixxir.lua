...
local utilities = {
	...
	get_item_text = function(item_name)
		return "\a666666FF‹\aCCBBFFFFNutahook ebanaya pasta\a666666FF› \a909090FF" .. item_name
	end,
		...
		menu = {
			...
			info = {
				text = ui.new_label(
					"AA",
					"Fake lag",
					"\aCCBBFFFFnutahook\a666666FF\aCCBBFFFF \a909090FF- the shit anti-aim lua"
				),
				text2 = ui.new_label("AA", "Fake lag", "\aCCBBFFFFyour username \a909090FF- crack by kubisshvh"),
				text3 = ui.new_label("AA", "Fake lag", "\aCCBBFFFFversion \a909090FF- kall"),
			},
			antiaim = {
...
ui.set_visible(nutahook.variables.references.misc.settings.sv_maxusrcmdprocessticks, true)
ui.set_visible(nutahook.variables.references.misc.settings.sv_maxunlag, true)
local functions = {
	menu = {
		main = {
			update = function()
				ui.set_visible(nutahook.variables.menu.tab, ui.get(nutahook.variables.menu.enable))

				ui.set_visible(
					nutahook.variables.menu.info.text,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "info"
				)
				ui.set_visible(
					nutahook.variables.menu.info.text2,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "info"
				)
				ui.set_visible(
					nutahook.variables.menu.info.text3,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "info"
				)

				ui.set_visible(
					nutahook.variables.menu.antiaim.enable,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "anti-aim"
				)

				ui.set_visible(
					nutahook.variables.menu.visuals.enable,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "visuals"
				)

				ui.set_visible(
					nutahook.variables.menu.miscellaneous.enable,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "miscellaneous"
				)

				ui.set_visible(
					nutahook.variables.menu.configs.import,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "configs"
				)
				ui.set_visible(
					nutahook.variables.menu.configs.export,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "configs"
				)
				ui.set_visible(
					nutahook.variables.menu.configs.default,
					ui.get(nutahook.variables.menu.enable) and ui.get(nutahook.variables.menu.tab) == "configs"
				)
			end,
		},
		antiaim = {
			update = function()
				local new_visibility = ui.get(nutahook.variables.menu.enable)
					and ui.get(nutahook.variables.menu.tab) == "anti-aim"
					and ui.get(nutahook.variables.menu.antiaim.enable)

				ui.set_visible(nutahook.variables.menu.antiaim.keybinds.freestanding, new_visibility)
				ui.set_visible(nutahook.variables.menu.antiaim.keybinds.freestanding_bind, new_visibility)

				ui.set_visible(nutahook.variables.menu.antiaim.keybinds.man_enable, new_visibility)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_forward,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_back,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_left,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_right,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_jitter,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_attargets,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)
				ui.set_visible(
					nutahook.variables.menu.antiaim.keybinds.man_builtinind,
					new_visibility and ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
				)

				ui.set_visible(nutahook.variables.menu.antiaim.condition_label, new_visibility)
				ui.set_visible(nutahook.variables.menu.antiaim.condition, new_visibility)
				ui.set_visible(nutahook.variables.menu.antiaim.condition_count, false)
				ui.set_visible(nutahook.variables.menu.antiaim.update_names, new_visibility)
				ui.set_visible(nutahook.variables.menu.antiaim.delete_condition, new_visibility)
				-- for k, v in pairs(nutahook.states) do
				-- 	local default_visibility = new_visibility and ui.get(nutahook.variables.menu.antiaim.state) == v
				-- 	local state_visibility = default_visibility
				-- 	if v ~= "default" then
				-- 		state_visibility = state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].enable)

				-- 		ui.set_visible(nutahook.variables.menu.antiaim.builder[v].enable, default_visibility)
				-- 	end

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].pitch, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].custom_pitch,
				-- 		state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].pitch) == "custom")

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].yaw_base, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].yaw, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].offset_l,
				-- 		state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].yaw) ~= "off")
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].offset_r,
				-- 		state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].yaw) ~= "off")

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].yaw_jitter, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].jitter_amount,
				-- 		state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].yaw_jitter) ~= "off")

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].body_yaw, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].body_yaw_amount,
				-- 		state_visibility and ui.get(nutahook.variables.menu.antiaim.builder[v].body_yaw) ~= "off" and
				-- 		ui.get(nutahook.variables.menu.antiaim.builder[v].body_yaw) ~= "opposite")

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].freestanding_body_yaw, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].edge_yaw, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].roll, state_visibility)

				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].fl_enable, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].fl_amount, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].fl_variance, state_visibility)
				-- 	ui.set_visible(nutahook.variables.menu.antiaim.builder[v].fl_limit, state_visibility)
				-- end
			end,
		},
		visuals = {
			update = function()
				local vis = ui.get(nutahook.variables.menu.enable)
					and ui.get(nutahook.variables.menu.tab) == "visuals"
					and ui.get(nutahook.variables.menu.visuals.enable)

				ui.set_visible(nutahook.variables.menu.visuals.style, vis)
				ui.set_visible(
					nutahook.variables.menu.visuals.breath_speed,
					vis and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)

				ui.set_visible(nutahook.variables.menu.visuals.watermark, vis)
				ui.set_visible(
					nutahook.variables.menu.visuals.watermark_color,
					vis and ui.get(nutahook.variables.menu.visuals.watermark)
				)

				ui.set_visible(
					nutahook.variables.menu.visuals.indicators,
					vis and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.indicators_color,
					vis
						and ui.get(nutahook.variables.menu.visuals.indicators)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.indicators_elements,
					vis
						and ui.get(nutahook.variables.menu.visuals.indicators)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)

				ui.set_visible(nutahook.variables.menu.visuals.slowdown, vis)
				ui.set_visible(
					nutahook.variables.menu.visuals.slowdown_color,
					vis and ui.get(nutahook.variables.menu.visuals.slowdown)
				)

				ui.set_visible(
					nutahook.variables.menu.visuals.low_ammo,
					vis and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.low_ammo_color,
					vis
						and ui.get(nutahook.variables.menu.visuals.low_ammo)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.low_ammo_threshold,
					vis
						and ui.get(nutahook.variables.menu.visuals.low_ammo)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				...
				ui.set_visible(
					nutahook.variables.menu.visuals.low_hp,
					vis and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.low_hp_color,
					vis
						and ui.get(nutahook.variables.menu.visuals.low_hp)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.low_hp_threshold,
					vis
						and ui.get(nutahook.variables.menu.visuals.low_hp)
						and ui.get(nutahook.variables.menu.visuals.style) == "new pixel"
				)

				ui.set_visible(nutahook.variables.menu.visuals.bloom, vis)
				ui.set_visible(
					nutahook.variables.menu.visuals.bloom_amount,
					vis and ui.get(nutahook.variables.menu.visuals.bloom)
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.bloom_red_mul,
					vis and ui.get(nutahook.variables.menu.visuals.bloom)
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.bloom_green_mul,
					vis and ui.get(nutahook.variables.menu.visuals.bloom)
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.bloom_blue_mul,
					vis and ui.get(nutahook.variables.menu.visuals.bloom)
				)

				ui.set_visible(nutahook.variables.menu.visuals.aspect_ratio, vis)
				ui.set_visible(
					nutahook.variables.menu.visuals.ar_mode,
					vis and ui.get(nutahook.variables.menu.visuals.aspect_ratio)
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.ar_float,
					vis
						and ui.get(nutahook.variables.menu.visuals.aspect_ratio)
						and ui.get(nutahook.variables.menu.visuals.ar_mode) == "float"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.ar_x,
					vis
						and ui.get(nutahook.variables.menu.visuals.aspect_ratio)
						and ui.get(nutahook.variables.menu.visuals.ar_mode) == "display size"
				)
				ui.set_visible(
					nutahook.variables.menu.visuals.ar_y,
					vis
						and ui.get(nutahook.variables.menu.visuals.aspect_ratio)
						and ui.get(nutahook.variables.menu.visuals.ar_mode) == "display size"
				)
		...
		miscellaneous = {
			update = function()
				local vis = ui.get(nutahook.variables.menu.enable)
					and ui.get(nutahook.variables.menu.tab) == "miscellaneous"
					and ui.get(nutahook.variables.menu.miscellaneous.enable)

				ui.set_visible(nutahook.variables.menu.miscellaneous.aimbot_logs, vis)

				ui.set_visible(nutahook.variables.menu.miscellaneous.clantag, vis)
				ui.set_visible(nutahook.variables.menu.miscellaneous.killsay, vis)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.killsay_delay,
					vis and ui.get(nutahook.variables.menu.miscellaneous.killsay)
				)

				ui.set_visible(nutahook.variables.menu.miscellaneous.rename_md, vis)
				ui.set_visible(nutahook.variables.menu.miscellaneous.fast_ladder, vis)
				ui.set_visible(nutahook.variables.menu.miscellaneous.anti_backstab, vis)

				ui.set_visible(nutahook.variables.menu.miscellaneous.resolver, vis)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.resolve_only_target,
					vis and ui.get(nutahook.variables.menu.miscellaneous.resolver)
				)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.resolve_teammates,
					vis
						and ui.get(nutahook.variables.menu.miscellaneous.resolver)
						and not ui.get(nutahook.variables.menu.miscellaneous.resolve_only_target)
				)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.resolver_esp_flag,
					vis and ui.get(nutahook.variables.menu.miscellaneous.resolver)
				)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.resolver_jitter_detection,
					vis and ui.get(nutahook.variables.menu.miscellaneous.resolver)
				)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.resolver_version,
					vis and ui.get(nutahook.variables.menu.miscellaneous.resolver)
				)

				ui.set_visible(nutahook.variables.menu.miscellaneous.exploits.enable, vis)

				ui.set_visible(
					nutahook.variables.menu.miscellaneous.exploits.flag,
					vis and ui.get(nutahook.variables.menu.miscellaneous.exploits.enable)
				)
				ui.set_visible(
					nutahook.variables.menu.miscellaneous.exploits.flag_bind,
					vis
						and ui.get(nutahook.variables.menu.miscellaneous.exploits.flag)
						and ui.get(nutahook.variables.menu.miscellaneous.exploits.enable)
				)
			end,
		},
	},
	skeet_menu = {
		set_visible = function(val)
			ui.set_visible(nutahook.variables.references.antiaim.enable, val)

			ui.set_visible(nutahook.variables.references.antiaim.pitch[1], val)
			ui.set_visible(
				nutahook.variables.references.antiaim.pitch[2],
				val and nutahook.variables.references.antiaim.pitch[1] == "custom"
			)
			ui.set_visible(nutahook.variables.references.antiaim.yaw_base, val)

			ui.set_visible(nutahook.variables.references.antiaim.yaw[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.yaw[2], val)
			ui.set_visible(nutahook.variables.references.antiaim.yaw_jitter[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.yaw_jitter[2], val)
			ui.set_visible(nutahook.variables.references.antiaim.body_yaw[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.body_yaw[2], val)

			ui.set_visible(nutahook.variables.references.antiaim.freestanding_body_yaw, val)
			ui.set_visible(nutahook.variables.references.antiaim.freestanding[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.freestanding[2], val)

			ui.set_visible(nutahook.variables.references.antiaim.edge_yaw, val)
			ui.set_visible(nutahook.variables.references.antiaim.roll, val)

			ui.set_visible(nutahook.variables.references.antiaim.fake_lag.enable[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.fake_lag.enable[2], val)
			ui.set_visible(nutahook.variables.references.antiaim.fake_lag.amount, val)
			ui.set_visible(nutahook.variables.references.antiaim.fake_lag.variance, val)
			ui.set_visible(nutahook.variables.references.antiaim.fake_lag.limit, val)

			ui.set_visible(nutahook.variables.references.antiaim.other.slow_motion[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.other.slow_motion[2], val)
			ui.set_visible(nutahook.variables.references.antiaim.other.leg_movement, val)
			ui.set_visible(nutahook.variables.references.antiaim.other.on_shot_aa[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.other.on_shot_aa[2], val)
			ui.set_visible(nutahook.variables.references.antiaim.other.fake_peek[1], val)
			ui.set_visible(nutahook.variables.references.antiaim.other.fake_peek[2], val)
		end,
	},
	antiaim = {
		update_names = function()
			local new_states = nutahook.variables.features.antiaim.states
			for k, v in pairs(new_states) do
				if k ~= 1 then
					new_states[k] = ui.get(nutahook.variables.menu.antiaim.builder[k].name) .. " [" .. k - 1 .. "]"
				end
			end
			local menu_conditions = utilities.copy_table(new_states)
			table.insert(menu_conditions, #menu_conditions + 1, "+ new")
			ui.update(nutahook.variables.menu.antiaim.condition, menu_conditions)
		end,
		delete_condition = function(cond)
			if cond == 0 then
				return
			end
			if cond == #nutahook.variables.features.antiaim.states then
				return
			end
			...
			nutahook.variables.features.antiaim.states = new_states

			local menu_conditions = utilities.copy_table(new_states)
			table.insert(menu_conditions, #menu_conditions + 1, "+ new")
			ui.update(nutahook.variables.menu.antiaim.condition, menu_conditions)

			ui.set(nutahook.variables.menu.antiaim.condition, #menu_conditions - 2)
			ui.set(
				nutahook.variables.menu.antiaim.condition_count,
				ui.get(nutahook.variables.menu.antiaim.condition_count) - 1
			)
		end,
		create_condition = function()
			...
			nutahook.variables.features.antiaim.states = new_states
			nutahook.variables.features.antiaim.max_states =
				math.max(nutahook.variables.features.antiaim.max_states, #new_states)
			...
			nutahook.variables.menu.antiaim.builder[v] = {}
			nutahook.variables.menu.antiaim.builder[v].when = {}

			nutahook.variables.menu.antiaim.builder[v].when.activators =
			...
			nutahook.variables.menu.antiaim.builder[v].when.min_velocity =
				ui.new_slider("AA", "Fake lag", utilities.get_cond_text("when", "min velocity"), 0, 400, 0)
			nutahook.variables.menu.antiaim.builder[v].when.max_velocity =
				ui.new_slider("AA", "Fake lag", utilities.get_cond_text("when", "max velocity"), 0, 400, 0)
			nutahook.variables.menu.antiaim.builder[v].when.airbourne =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "airbourne"))
			nutahook.variables.menu.antiaim.builder[v].when.team = ui.new_combobox(
				"AA",
				"Fake lag",
				utilities.get_cond_text("when", "team"),
				{ "counter-terrorists", "terrorists" }
			)
			nutahook.variables.menu.antiaim.builder[v].when.warmup =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "warmup"))
			nutahook.variables.menu.antiaim.builder[v].when.crouching =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "crouching"))
			nutahook.variables.menu.antiaim.builder[v].when.doubletap =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "doubletap"))
			nutahook.variables.menu.antiaim.builder[v].when.hideshots =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "hideshots"))
			nutahook.variables.menu.antiaim.builder[v].when.slow_motion =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "slow motion"))
			nutahook.variables.menu.antiaim.builder[v].when.quick_peek_assist =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "quick peek assist"))
			nutahook.variables.menu.antiaim.builder[v].when.duck_peek_assist =
				ui.new_checkbox("AA", "Fake lag", utilities.get_cond_text("when", "duck peek assist"))
			nutahook.variables.menu.antiaim.builder[v].when.keybind =
				ui.new_hotkey("AA", "Fake lag", utilities.get_cond_text("when", "keybind"))

			nutahook.variables.menu.antiaim.builder[v].name_label =
				ui.new_label("AA", "Anti-aimbot angles", utilities.get_cond_text("anti-aim", "condition name"))
			nutahook.variables.menu.antiaim.builder[v].name = ui.new_textbox("AA", "Anti-aimbot angles", "name")

			nutahook.variables.menu.antiaim.builder[v].pitch = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "pitch"),
				{ "off", "default", "up", "down", "minimal", "random", "custom" }
			)
			nutahook.variables.menu.antiaim.builder[v].custom_pitch = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "custom pitch"),
				-89,
			...
			nutahook.variables.menu.antiaim.builder[v].yaw_base = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "yaw base"),
				{ "local view", "at targets" }
			)
			nutahook.variables.menu.antiaim.builder[v].yaw = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "yaw"),
				{ "off", "180", "spin", "static", "180 Z", "crosshair" }
			)
			nutahook.variables.menu.antiaim.builder[v].offset_l = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "offset left"),
				-180,
			...
			nutahook.variables.menu.antiaim.builder[v].offset_r = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "offset right"),
				-180,
			...
			nutahook.variables.menu.antiaim.builder[v].yaw_jitter = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "jitter"),
				{ "off", "offset", "center", "random", "skitter" }
			)
			nutahook.variables.menu.antiaim.builder[v].jitter_amount = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "jitter amount"),
				-180,
			...
			nutahook.variables.menu.antiaim.builder[v].body_yaw = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "body yaw"),
				{ "off", "opposite", "jitter", "static" }
			)
			nutahook.variables.menu.antiaim.builder[v].body_yaw_amount = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "body yaw amount"),
				-180,
			...
			nutahook.variables.menu.antiaim.builder[v].freestanding_body_yaw = ui.new_checkbox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("anti-aim", "freestanding body yaw")
			)
			nutahook.variables.menu.antiaim.builder[v].edge_yaw =
				ui.new_checkbox("AA", "Anti-aimbot angles", utilities.get_cond_text("anti-aim", "edge yaw"))

			nutahook.variables.menu.antiaim.builder[v].roll =
				ui.new_slider("AA", "Anti-aimbot angles", utilities.get_cond_text("anti-aim", "roll"), -45, 45, 0)

			nutahook.variables.menu.antiaim.builder[v].fl_enable =
				ui.new_checkbox("AA", "Anti-aimbot angles", utilities.get_cond_text("fake-lag", "enable"))
			nutahook.variables.menu.antiaim.builder[v].fl_amount = ui.new_combobox(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("fake-lag", "amount"),
				{ "dynamic", "maximum", "fluctuate" }
			)
			nutahook.variables.menu.antiaim.builder[v].fl_variance = ui.new_slider(
				"AA",
				"Anti-aimbot angles",
				utilities.get_cond_text("fake-lag", "variance"),
				0,
			...
			nutahook.variables.menu.antiaim.builder[v].fl_limit =
		...
		get_state_id = function(player_id)
			...
			local doubletap = ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[1])
				and ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[2])
			local hideshots = ui.get(nutahook.variables.references.antiaim.other.on_shot_aa[1])
				and ui.get(nutahook.variables.references.antiaim.other.on_shot_aa[2])

			local slow_motion = ui.get(nutahook.variables.references.antiaim.other.slow_motion[1])
				and ui.get(nutahook.variables.references.antiaim.other.slow_motion[2])

			local quick_peek_assist = ui.get(nutahook.variables.references.ragebot.other.quick_peek_assist)
			local duck_peek_assist = ui.get(nutahook.variables.references.ragebot.other.duck_peek_assist)

			for k, v in pairs(nutahook.variables.features.antiaim.states) do
				if k == 1 then
					goto continue
				end
				local active = false
				local v = nutahook.variables.menu.antiaim.builder[k]
	...
	config = {
		export = function()
			...
			local config_table = export_table(nutahook.variables.menu)
...
functions.config.import = function(config)
	...
	if count ~= nil then
		while
			ui.get(nutahook.variables.menu.antiaim.condition_count) < count
			or #nutahook.variables.menu.antiaim.builder < count + 1
		do
	...
	import_table(js, nutahook.variables.menu)
	client.delay_call(1, function()
		functions.antiaim.update_names()
	end)
end
nutahook.variables.menu.antiaim.update_names = ui.new_button("AA", "Fake lag", "update names", function()
	functions.antiaim.update_names()
end)
nutahook.variables.menu.antiaim.delete_condition = ui.new_button("AA", "Fake lag", "delete", function()
	functions.antiaim.delete_condition(ui.get(nutahook.variables.menu.antiaim.condition))
end)
nutahook.variables.menu.configs.import = ui.new_button("AA", "Anti-aimbot angles", "import from clipboard", function()
	functions.config.import(libraries.clipboard.get())
end)
nutahook.variables.menu.configs.export = ui.new_button("AA", "Anti-aimbot angles", "export to clipboard", function()
	libraries.clipboard.set(functions.config.export())
end)
nutahook.variables.menu.configs.default = ui.new_button(
	"AA",
	"Anti-aimbot angles",
	"default config (ebani kall)",
	function()
...
local features = {
	antiaim = {
		update_builder = function()
			local states = nutahook.variables.features.antiaim.states
			if ui.get(nutahook.variables.menu.antiaim.condition) > #states then
				ui.set(nutahook.variables.menu.antiaim.condition, 0)
			end
			if ui.get(nutahook.variables.menu.antiaim.condition) == #states then
				functions.antiaim.create_condition()
			end
			...
			local vis = ui.get(nutahook.variables.menu.enable)
				and ui.get(nutahook.variables.menu.tab) == "anti-aim"
				and ui.get(nutahook.variables.menu.antiaim.enable)
			local cur_state = ui.get(nutahook.variables.menu.antiaim.condition) + 1
			for k = 1, nutahook.variables.features.antiaim.max_states do
				local state = k == 1 and "default" or k
				local reftable = nutahook.variables.menu.antiaim.builder[state]
		...
		handle_builder = function()
			-- ui.set(nutahook.variables.references.antiaim.enable, ui.get(nutahook.variables.menu.antiaim.enable))

			if not ui.get(nutahook.variables.menu.antiaim.enable) then
				return
			end
			local fake = utilities.get_fake_amount(entity.get_local_player())
			nutahook.variables.features.antiaim.fake = fake

			local state = functions.antiaim.get_state_id(entity.get_local_player())

			local manual = -1
			if ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable) then
				local forward = ui.get(nutahook.variables.menu.antiaim.keybinds.man_forward)
				local back = ui.get(nutahook.variables.menu.antiaim.keybinds.man_back)
				local left = ui.get(nutahook.variables.menu.antiaim.keybinds.man_left)
				local right = ui.get(nutahook.variables.menu.antiaim.keybinds.man_right)
			...
			local fs = ui.get(nutahook.variables.menu.antiaim.keybinds.freestanding)
				and ui.get(nutahook.variables.menu.antiaim.keybinds.freestanding_bind)
			ui.set(nutahook.variables.references.antiaim.freestanding[1], fs)
			ui.set(nutahook.variables.references.antiaim.freestanding[2], fs and "Always On" or "On hotkey")

			ui.set(
				nutahook.variables.references.antiaim.pitch[1],
				ui.get(nutahook.variables.menu.antiaim.builder[state].pitch)
			)
			ui.set(
				nutahook.variables.references.antiaim.pitch[2],
				ui.get(nutahook.variables.menu.antiaim.builder[state].custom_pitch)
			)

			if manual == -1 then
				ui.set(
					nutahook.variables.references.antiaim.yaw_base,
					ui.get(nutahook.variables.menu.antiaim.builder[state].yaw_base)
				)

				ui.set(
					nutahook.variables.references.antiaim.yaw[1],
					ui.get(nutahook.variables.menu.antiaim.builder[state].yaw)
				)
				...
				if fake > 0 then
					ui.set(
						nutahook.variables.references.antiaim.yaw[2],
						ui.get(nutahook.variables.menu.antiaim.builder[state].offset_l) + additional_yaw
					)
				else
					ui.set(
						nutahook.variables.references.antiaim.yaw[2],
						ui.get(nutahook.variables.menu.antiaim.builder[state].offset_r) + additional_yaw
					)
				end
			else
				ui.set(
					nutahook.variables.references.antiaim.yaw_base,
					ui.get(nutahook.variables.menu.antiaim.keybinds.man_attargets) and "at targets" or "local view"
				)
				ui.set(nutahook.variables.references.antiaim.yaw[1], "180")
				ui.set(nutahook.variables.references.antiaim.yaw[2], manual)
			end

			if manual == -1 or ui.get(nutahook.variables.menu.antiaim.keybinds.man_jitter) then
				ui.set(
					nutahook.variables.references.antiaim.yaw_jitter[1],
					ui.get(nutahook.variables.menu.antiaim.builder[state].yaw_jitter)
				)
				ui.set(
					nutahook.variables.references.antiaim.yaw_jitter[2],
					ui.get(nutahook.variables.menu.antiaim.builder[state].jitter_amount)
				)
			end

			ui.set(
				nutahook.variables.references.antiaim.body_yaw[1],
				ui.get(nutahook.variables.menu.antiaim.builder[state].body_yaw)
			)
			ui.set(
				nutahook.variables.references.antiaim.body_yaw[2],
				ui.get(nutahook.variables.menu.antiaim.builder[state].body_yaw_amount)
			)

			ui.set(
				nutahook.variables.references.antiaim.freestanding_body_yaw,
				ui.get(nutahook.variables.menu.antiaim.builder[state].freestanding_body_yaw)
			)
			ui.set(
				nutahook.variables.references.antiaim.edge_yaw,
				ui.get(nutahook.variables.menu.antiaim.builder[state].edge_yaw) and manual == -1
			)
			ui.set(nutahook.variables.references.antiaim.roll, ui.get(nutahook.variables.menu.antiaim.builder[state].roll))

			ui.set(
				nutahook.variables.references.antiaim.fake_lag.enable[1],
				ui.get(nutahook.variables.menu.antiaim.builder[state].fl_enable)
			)
			ui.set(nutahook.variables.references.antiaim.fake_lag.enable[2], "Always on")
			ui.set(
				nutahook.variables.references.antiaim.fake_lag.amount,
				ui.get(nutahook.variables.menu.antiaim.builder[state].fl_amount)
			)
			ui.set(
				nutahook.variables.references.antiaim.fake_lag.variance,
				ui.get(nutahook.variables.menu.antiaim.builder[state].fl_variance)
			)
			ui.set(
				nutahook.variables.references.antiaim.fake_lag.limit,
				ui.get(nutahook.variables.menu.antiaim.builder[state].fl_limit)
			)
		end,
	},
	visuals = {
		slowdown_indicator = {
			handle = function()
				if
					not ui.get(nutahook.variables.menu.visuals.slowdown)
					or not ui.get(nutahook.variables.menu.visuals.enable)
				then
					return
				end
				...
				local anim = nutahook.variables.features.visuals.slowdown_indicator.anim
				...
				nutahook.variables.features.visuals.slowdown_indicator.anim = anim
				...
				local style = ui.get(nutahook.variables.menu.visuals.style)
				local r, g, b, a = ui.get(nutahook.variables.menu.visuals.slowdown_color)
				...
				elseif style == "new pixel" then
					...
					local anim_breathing = nutahook.variables.features.visuals.anim_breathing
					local text = "SLOWED  DOWN  #  " .. math.floor(modifier * 100) .. "%"
					local tx, ty = renderer.measure_text("c-", text)

					utilities.glow(
						mid_x - tx / 2 + 1,
						mid_y - 120 - nutahook.variables.features.visuals.warnings_offset - animpos - ty / 2 + 2,
						tx + 1,
						ty - 4,
						7,
						2,
					...
					renderer.text(
						mid_x,
						mid_y - 120 - nutahook.variables.features.visuals.warnings_offset - animpos,
						r,
						g,
						b,
					...
					nutahook.variables.features.visuals.warnings_offset = nutahook.variables.features.visuals.warnings_offset
						+ 10
				end
			end,
		},
		low_ammo_warning = {
			handle = function()
				if
					not ui.get(nutahook.variables.menu.visuals.low_ammo)
					or not ui.get(nutahook.variables.menu.visuals.enable)
				then
					return
				end
				...
				local show = (ammo / max_ammo * 100.0) <= ui.get(nutahook.variables.menu.visuals.low_ammo_threshold)
					and entity.is_alive(entity.get_local_player())
				if ui.is_menu_open() then
					show = true
				end

				local anim = nutahook.variables.features.visuals.low_ammo.anim
				if show and anim < 1.0 then
					anim = anim + 4.7 * globals.frametime()
				elseif not show and anim > 0.0 then
					anim = anim - 4.7 * globals.frametime()
				...
				anim = utilities.clamp(anim, 0, 1)
				nutahook.variables.features.visuals.low_ammo.anim = anim
				...
				local style = ui.get(nutahook.variables.menu.visuals.style)
				local r, g, b, a = ui.get(nutahook.variables.menu.visuals.low_ammo_color)
		...
		low_hp_warning = {
			handle = function()
				if
					not ui.get(nutahook.variables.menu.visuals.low_hp) or not ui.get(nutahook.variables.menu.visuals.enable)
				then
					return
				end
				...
				local show = health <= ui.get(nutahook.variables.menu.visuals.low_hp_threshold)
					and entity.is_alive(entity.get_local_player())
				if ui.is_menu_open() then
					show = true
				end

				local anim = nutahook.variables.features.visuals.low_hp.anim
				if show and anim < 1.0 then
					anim = anim + 4.7 * globals.frametime()
				elseif not show and anim > 0.0 then
					anim = anim - 4.7 * globals.frametime()
				...
				anim = utilities.clamp(anim, 0, 1)
				nutahook.variables.features.visuals.low_hp.anim = anim
				...
				local style = ui.get(nutahook.variables.menu.visuals.style)
				local r, g, b, a = ui.get(nutahook.variables.menu.visuals.low_hp_color)
		...
		watermark = {
			handle = function()
				if
					not ui.get(nutahook.variables.menu.visuals.watermark)
					or not ui.get(nutahook.variables.menu.visuals.enable)
				then
					return
				end
				...
				local style = ui.get(nutahook.variables.menu.visuals.style)
				local r, g, b, a = ui.get(nutahook.variables.menu.visuals.watermark_color)

				if style == "old" then
					local watermark_text = "nutahook.tech"
				...
				elseif style == "new pixel" then
					local watermark_text = string.format(
						"\a666666FF‹\a%02x%02x%02xFFnutahook\a666666FF.\a%02x%02x%02xFFTECH \a666666FF›",
						r,
						g,
						b,
						r,
		...
		manual_indicators = {
			handle_skeet = function()
				if
					not ui.get(nutahook.variables.menu.antiaim.enable)
					or not ui.get(nutahook.variables.menu.antiaim.keybinds.man_enable)
					or not ui.get(nutahook.variables.menu.antiaim.keybinds.man_builtinind)
				then
					return
				end
				local forward = ui.get(nutahook.variables.menu.antiaim.keybinds.man_forward)
		...
		crosshair_indicators = {
			handle = function()
				...
				local show_anim = nutahook.variables.features.visuals.crosshair_indicators.anims.show
				do
					...
					nutahook.variables.features.visuals.crosshair_indicators.anims.show = show_anim
				end

				local scope_anim = nutahook.variables.features.visuals.crosshair_indicators.anims.scope
				do
					local scoped = nutahook.variables.features.visuals.crosshair_indicators.scoped
					...
					nutahook.variables.features.visuals.crosshair_indicators.anims.scope = scope_anim
				end
				...
				local style = ui.get(nutahook.variables.menu.visuals.style)
				local r, g, b, a = ui.get(nutahook.variables.menu.visuals.indicators_color)

				local state = "DEFAULT"
				local show_state = false
				local elements_parsed = {}
				do
					local elements = ui.get(nutahook.variables.menu.visuals.indicators_elements)
					for k, v in pairs(elements) do
						...
						elseif v == "doubletap" then
							str = "DT"
							show = ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[1])
								and ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[2])
						elseif v == "on-shot" then
							local dt = ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[1])
								and ui.get(nutahook.variables.references.ragebot.aimbot.double_tap[2])
							local hs = ui.get(nutahook.variables.references.antiaim.other.on_shot_aa[1])
								and ui.get(nutahook.variables.references.antiaim.other.on_shot_aa[2])
							str = "OS"
							show = not dt and hs
						elseif v == "minimum damage" then
							local dmg = ui.get(nutahook.variables.references.ragebot.aimbot.damage_override[3])
							if dmg < 100 then
								str = "DMG (" .. dmg .. ")"
							elseif dmg == 100 then
								str = "DMG (hp)"
							...
							show = ui.get(nutahook.variables.references.ragebot.aimbot.damage_override[1])
								and ui.get(nutahook.variables.references.ragebot.aimbot.damage_override[2])
						end
				...
				if style == "new pixel" then
					...
					do -- light anim
						...
						local light_pos = nutahook.variables.features.visuals.crosshair_indicators.light_pos
						local fake = nutahook.variables.features.visuals.crosshair_indicators.fake
						if fake > 0 and light_pos > 0.0 then
							light_pos = light_pos - frame * 6.7
						elseif fake < 0 and light_pos < 1.0 then
							light_pos = light_pos + frame * 6.7
						else
							nutahook.variables.features.visuals.crosshair_indicators.fake =
								nutahook.variables.features.antiaim.fake
						end
						light_pos = utilities.clamp(light_pos, 0, 1)
						nutahook.variables.features.visuals.crosshair_indicators.light_pos = light_pos

						local chars = "nutahook"
						for i = 1, #chars do
	...
	miscellaneous = {
		aimbot_logs = {
			net_update_end = function()
				...
				nutahook.variables.features.miscellaneous.aimbot_logs.cl_data.tick_shifted = false

				if m_tick_base ~= nil then
					if
						nutahook.variables.features.miscellaneous.aimbot_logs.cl_data.tick_base ~= 0
						and m_tick_base < nutahook.variables.features.miscellaneous.aimbot_logs.cl_data.tick_base
					then
						nutahook.variables.features.miscellaneous.aimbot_logs.cl_data.tick_shifted = true
					end

					nutahook.variables.features.miscellaneous.aimbot_logs.cl_data.tick_base = m_tick_base
				end

				for i = 1, #players do
					local idx = players[i]
					local prev_tick = nutahook.variables.features.miscellaneous.aimbot_logs.sim_ticks[idx]

					if entity.is_dormant(idx) or not entity.is_alive(idx) then
						nutahook.variables.features.miscellaneous.aimbot_logs.sim_ticks[idx] = nil
						nutahook.variables.features.miscellaneous.aimbot_logs.net_data[idx] = nil
					else
						...
						if prev_tick ~= nil then
							...
							if delta < 0 or delta > 0 and delta <= 64 then
								...
								nutahook.variables.features.miscellaneous.aimbot_logs.net_data[idx] = {
									tick = delta - 1,
						...
						nutahook.variables.features.miscellaneous.aimbot_logs.sim_ticks[idx] = {
			...
			fire = function(event)
				...
				local force_safe_point = ui.get(nutahook.variables.references.ragebot.aimbot.force_safe_point)

				if nutahook.variables.features.miscellaneous.aimbot_logs.net_data[event.target] == nil then
					nutahook.variables.features.miscellaneous.aimbot_logs.net_data[event.target] = {}
				end

				data.teleported = nutahook.variables.features.miscellaneous.aimbot_logs.net_data[event.target].lagcomp
					or false
				data.choke = nutahook.variables.features.miscellaneous.aimbot_logs.net_data[event.target].tick or "?"
				...
				nutahook.variables.features.miscellaneous.aimbot_logs.data[event.id] = data
			end,
			hit = function(event)
				if
					not ui.get(nutahook.variables.menu.miscellaneous.aimbot_logs)
					or nutahook.variables.features.miscellaneous.aimbot_logs.data[event.id] == nil
				then
					return
				end

				local on_fire_data = nutahook.variables.features.miscellaneous.aimbot_logs.data[event.id]
				local name = string.lower(entity.get_player_name(event.target))
				local hgroup = nutahook.hitgroup_names[event.hitgroup + 1] or "unknown"
				local aimed_hgroup = nutahook.hitgroup_names[on_fire_data.hitgroup + 1] or "unknown"
		...
		clantag = {
			handle = function()
				if
					ui.get(nutahook.variables.menu.miscellaneous.enable)
					and ui.get(nutahook.variables.menu.miscellaneous.clantag)
				then
					local tick = math.floor(globals.tickcount() / 75)
					local tag = tick % #nutahook.clantag + 1

					local clantag = nutahook.clantag[tag]

					if clantag == nutahook.variables.features.miscellaneous.clantag_last then
						return
					end
					nutahook.variables.features.miscellaneous.clantag_last = clantag
					nutahook.variables.features.miscellaneous.clantag_back = true

					client.set_clan_tag(clantag)
				elseif nutahook.variables.features.miscellaneous.clantag_back then
					client.set_clan_tag()
				end
			end,
		},
		killsay = {
			handle = function(event)
				if
					ui.get(nutahook.variables.menu.miscellaneous.enable)
					and ui.get(nutahook.variables.menu.miscellaneous.killsay)
				then
					...
					client.delay_call(ui.get(nutahook.variables.menu.miscellaneous.killsay_delay) / 10, function()
						client.exec("say ", nutahook.killsay[math.random(1, #nutahook.killsay)])
					end)
				end
			end,
		},
...
do -- resolver functions
	do -- Player
		...
		function features.miscellaneous.resolver.Player:DetectJitter()
			...
			local method = ui.get(nutahook.variables.menu.miscellaneous.resolver_jitter_detection)
		...
		function features.miscellaneous.resolver.Player:GetDesyncUnclamped()
			if ui.get(nutahook.variables.menu.miscellaneous.resolver_version) == "old" then
				...
				return 0
			elseif ui.get(nutahook.variables.menu.miscellaneous.resolver_version) == "stable" then
				local delta = math.rad(
					math.floor(
						math.abs(
				...
				return -ret
			elseif ui.get(nutahook.variables.menu.miscellaneous.resolver_version) == "experimental" then
				if entity.is_dormant(self.player) then
					return 0
				end
				local desync = 0
...
client.set_event_callback("paint_ui", function()
	functions.skeet_menu.set_visible(not ui.get(nutahook.variables.menu.enable))
...
client.set_event_callback("paint", function()
	nutahook.variables.features.visuals.warnings_offset = 0
	...
	do -- resolver data
		if
			ui.get(nutahook.variables.menu.miscellaneous.enable)
			and ui.get(nutahook.variables.menu.miscellaneous.resolver)
			and ui.get(nutahook.variables.menu.miscellaneous.resolver_version) == "experimental"
		then
			local x = 1600
			local y = 10
			...
			if not ui.get(nutahook.variables.menu.miscellaneous.resolve_only_target) then
				local players = entity.get_players(not ui.get(nutahook.variables.menu.miscellaneous.resolve_teammates))
	...
	do -- breathing anim
		local anim_breathing = nutahook.variables.features.visuals.anim_breathing
		local switch = nutahook.variables.features.visuals.breath_switch
		local breath_speed = ui.get(nutahook.variables.menu.visuals.breath_speed) / 6 + 0.4

		if anim_breathing <= 0.3 or anim_breathing >= 0.95 then
			switch = switch * -1
			nutahook.variables.features.visuals.breath_switch = switch
		end
		...
		nutahook.variables.features.visuals.anim_breathing = utilities.clamp(anim_breathing, 0.3, 1)
	end
end)

client.set_event_callback("pre_render", function()
	do
		local enabled = ui.get(nutahook.variables.menu.visuals.bloom) and ui.get(nutahook.variables.menu.visuals.enable)
		if enabled then
			ui.set(nutahook.variables.references.visuals.effects.disable_post_processing, false)
		end
		local bloom = enabled and (1.0 - ui.get(nutahook.variables.menu.visuals.bloom_amount) / 100.0) or 1.0
		local r_mul = enabled and ui.get(nutahook.variables.menu.visuals.bloom_red_mul) / 100.0 or 0.3
		local g_mul = enabled and ui.get(nutahook.variables.menu.visuals.bloom_green_mul) / 100.0 or 0.59
		local b_mul = enabled and ui.get(nutahook.variables.menu.visuals.bloom_blue_mul) / 100.0 or 0.11
		nutahook.variables.cvars.r_bloomtintexponent:set_float(bloom)
		nutahook.variables.cvars.r_bloomtintr:set_float(r_mul)
		nutahook.variables.cvars.r_bloomtintg:set_float(g_mul)
		nutahook.variables.cvars.r_bloomtintb:set_float(b_mul)
	end
	do
		local enabled = ui.get(nutahook.variables.menu.visuals.aspect_ratio)
			and ui.get(nutahook.variables.menu.visuals.enable)
		local mode = ui.get(nutahook.variables.menu.visuals.ar_mode)
		local flt = 0

		if mode == "float" then
			flt = ui.get(nutahook.variables.menu.visuals.ar_float) / 100
		elseif mode == "display size" then
			local x = ui.get(nutahook.variables.menu.visuals.ar_x)
			local y = ui.get(nutahook.variables.menu.visuals.ar_y)
			flt = x / (y == 0 and y + 1 or y)
		end

		nutahook.variables.cvars.r_aspectratio:set_float(flt)
	end
end)
...
client.set_event_callback("run_command", function(cmd)
	...
	nutahook.variables.features.visuals.crosshair_indicators.scoped = entity.get_prop(
...
client.set_event_callback("net_update_start", function()
	if ui.get(nutahook.variables.menu.miscellaneous.enable) and ui.get(nutahook.variables.menu.miscellaneous.resolver) then
		if not ui.get(nutahook.variables.menu.miscellaneous.resolve_only_target) then
			local players = entity.get_players(not ui.get(nutahook.variables.menu.miscellaneous.resolve_teammates))
...
client.register_esp_flag("Desync (resolver)", 255, 255, 255, function(player)
	if player ~= client.current_threat() and ui.get(nutahook.variables.menu.miscellaneous.resolve_only_target) then
		return
	end

	if not entity.is_enemy(player) and not ui.get(nutahook.variables.menu.miscellaneous.resolve_teammates) then
		return
	end
	...
	return ui.get(nutahook.variables.menu.miscellaneous.resolver) and ui.get(
		nutahook.variables.menu.miscellaneous.resolver_esp_flag
	),
